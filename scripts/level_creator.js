...
{
    alert(command_list.join('\n'))
};

var pop_balls = function()
{
    if (balls.length > 1)
    {
        balls.pop()
...
var mouseUp = function (e) {
    if (e.which == 1 && mouse.isDown) {
        ...
        balls.push(new Ball(ballx, bally, ballradius, 0.7, 10, ballcolour));
        command_list.push('balls.push(new Ball('+ballx+', '+bally+', '+ballradius+', 0.7, 10, '+ballcolour+'));');
...
function loop() {
    ...
    ctx.fillStyle = 'white';
    ctx.font = "11pt Arial";
    ctx.fillText("Mouse X: " + mouse.x, 0, 16);
    ctx.fillText("Mouse Y: " + mouse.y, 0, 32);
    ctx.fillText("Ball colour:", 0, 48)
}
...
rooms.push(
    new Room(
        function()
        {
            ...
            balls.push(new Ball(500, 500, 10, 0.7, 10, "rgb(" + 50 + "," + 50 + "," + 200 + ")"));
        })
)
...
// Spawn player ball
balls.push(new Ball(0, 545, 10, 0.7, 10, "rgb(" + 255 + "," + 50 + "," + 50 + ")"));
